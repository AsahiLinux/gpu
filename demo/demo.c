		...
		struct agx_allocator *shaders)
{
	...
	/* Vertex stuff */
	uint8_t unk00[] = {
		0x00, 0x00, 0x01, 0x00, 0x80, 0x07, 0x00, 0x00, 0x07, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	};

	uint8_t unk001[] = {
		0x58, 0x07, 0x00, 0x00, 0x18, 0x00, 0x00, 0x00,
		0x58, 0x07, 0x00, 0x00, 0x28, 0x07, 0x00, 0x00
	};

	EMIT_WORDS(cmdbuf, unk00, sizeof(unk00));
	EMIT_ZERO_WORDS(cmdbuf, 4);
	EMIT_WORDS(cmdbuf, unk001, sizeof(unk001));
